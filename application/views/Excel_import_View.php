...
.field-height{	padding-top:0px;height:24px; font-size:12px	}
    ...
    <div class="content p-4">
        ...
        <div class="card mb-4" >
            ...
            <form action="index.php/Excel_import/upload" method="post" enctype="multipart/form-data">
...
<div class="container mt-4">
    <?php if(isset($uploadedData) && !empty($uploadedData)): ?>
        <h2 class="mt-4">Uploaded Data</h2>
        <table class="table table-bordered">
            <thead>
                <tr>
                    <th>PAN ID</th>
                    <th>Response Code</th>
                    <th>Credit Debit ID</th>
                    <th>Amount</th>
                    <th>Time</th>
                    <th>RRN</th>
                    <th>Last</th>
                    <th>?Response_Code_1</th>
                </tr>
            </thead>
            <tbody>
                <?php foreach($uploadedData as $row): ?>
                    <tr>
                        ...
                        <td><?= $row['Credit_Debit_ID'] ?></td>
                        <td><?= $row['Amount'] ?></td>
                        <td><?= $row['Time'] ?></td>
                        <td><?= $row['RRN'] ?></td>
                        <td><?= $row['Last'] ?></td>
                        <td><?= $row['Response_Code_1'] ?></td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
